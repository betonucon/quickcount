...
<head>
  ...
  <!-- iCheck -->
  <link rel="stylesheet" href="{{url(urlnya().'plugins/iCheck/square/blue.css')}}">
...
<div class="login-box">
  <div class="login-logo">
    <img src="{{paslon(1)}}" width="40%"><br>
    <a href="{{url('login')}}"><b>MULIA</b>No.1</a>
  </div>
  <!-- /.login-logo -->
  <div class="login-box-body">
    ...
    <form action="{{ route('login') }}" method="post">
      ...
      <div class="form-group has-feedback">
        <input type="text" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" placeholder="Username" value="{{ old('email') }}" required autofocus>
        @if ($errors->has('email'))
            <span class="invalid-feedback" role="alert">
                <strong><i>Username tidak terdaftar</i></strong>
            </span>
        @endif
      </div>
      <div class="form-group has-feedback">
        <input type="password" name="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" placeholder="Password" value="{{ old('password') }}" required>
        @if ($errors->has('password'))
            <span class="invalid-feedback" role="alert">
                <strong><i>Password Salah</i></strong>
            </span>
        @endif
      </div>
      <div class="row">
...
<!-- iCheck -->
<script src="{{url(urlnya().'plugins/iCheck/icheck.min.js')}}"></script>
<script>
  $(function () {
    $('input').iCheck({
